...
\todo[inline]{For the Overview Figure, ksl and wpb like the version that shows the galaxy light (currently Fig. 2).  Frank is responsible for updating the overview figures, and describing them in the text.  Bill is responsible for one or more snap shot  region figures to show what objects we select. Then in addition to the overview figure, we should show an enlargement of a smaller region in a multi-panel format, showing the subtracted data.  He is also responsible for the associated text. .} 
...
\todo[inline]{ksl - It might be worthwhile to have a few sentences of comparison here to our work on M83 where the Magellen seeing made if far easier to see extended emission, and in M33 of course where all of the SNRs are resolved.  This is about the limit of what we can do from the ground, particularly with the seeing we had with WYIN.  To do better, one needs better seeing.}
...
\todo[inline]{We need a consistent story about the HII regions.  We mention that we selected some O3 objects.  We got spectra of 24 of these.   We have another 41 spectra of HII regions along the slit.  We got very few HII regions just by chance, though these are included in the 41.  It's hard to actually parse out the history of which are which.}
